...
if (!APP_ID || !ADMIN_TOKEN) {
  console.error("Missing INSTANT_APP_ID or INSTANT_ADMIN_TOKEN in environment.");
  Deno.exit(1);
}
...
async function cleanup() {
  ...
  for (const profile of profiles) {
    ...
    } else if (profile.user && typeof profile.user === "object" && !Array.isArray(profile.user)) {
  ...
  for (const [uid, userProfiles] of profilesByUser.entries()) {
    if (userProfiles.length > 1) {
      ...
      console.log(`User ${uid} has ${userProfiles.length} profiles. Keeping ${keep.id} (created: ${keep.createdAt}).`);

      for (const p of toDelete) {
        console.log(` -> Marking for deletion: ${p.id} (created: ${p.createdAt})`);
  ...
  if (txs.length > 0) {
    ...
    for (let i = 0; i < txs.length; i += CHUNK_SIZE) {
      ...
      console.log(`Deleted chunk ${i / CHUNK_SIZE + 1} / ${Math.ceil(txs.length / CHUNK_SIZE)}`);
    }
